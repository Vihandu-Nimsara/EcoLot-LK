            ...
            <table class="risk-table">
                ...
                <?php foreach ($riskRules as $rule): ?>
                    <tr>
                        ...
                        <td><div class="rule-actions"><button class="table-action-btn" type="button" data-admin-dialog="view-rule" data-rule-id="<?= htmlspecialchars((string) $rule['id']) ?>" data-name="<?= htmlspecialchars($rule['category']) ?>" data-risk="<?= htmlspecialchars($rule['risk']) ?>" data-status="<?= htmlspecialchars($rule['status']) ?>" data-note="<?= htmlspecialchars($rule['note']) ?>">View</button><button class="table-action-btn" type="button" data-admin-dialog="edit-rule" data-rule-id="<?= htmlspecialchars((string) $rule['id']) ?>" data-name="<?= htmlspecialchars($rule['category']) ?>" data-condition="<?= htmlspecialchars($rule['condition']) ?>" data-note="<?= htmlspecialchars($rule['note']) ?>">Edit</button><button class="table-action-btn" type="button" data-admin-dialog="rule-status" data-rule-id="<?= htmlspecialchars((string) $rule['id']) ?>" data-action="<?= $rule['status'] === 'ACTIVE' ? 'Deactivate' : 'Activate' ?>"><?= $rule['status'] === 'ACTIVE' ? 'Deactivate' : 'Activate' ?></button></div></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
