...
foreach ($cases as [$change, $restore, $params, $label]) {
    ...
    if (in_array($label, ['Exact close', 'After close'], true)) $reject(fn () => $service->withdrawBid($user, $bidId), $label . ' withdraw');
    $exec($restore, $params);
}
...
$exec("UPDATE recycler_licenses SET license_status = 'REVOKED' WHERE recycler_user_id = ?", [$user]);
$exec("UPDATE recycler_capabilities SET capability_status = 'SUSPENDED' WHERE recycler_user_id = ?", [$user]);
$service->withdrawBid($user, $bidId);
...
$exec("UPDATE recycler_licenses SET license_status = 'VALID' WHERE recycler_user_id = ?", [$user]);
